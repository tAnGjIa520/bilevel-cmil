...
class WeightedTopKSelector(nn.Module):
    ...
    def update_temperature(self, new_temperature):
        ...
        This method updates both the local temperature attribute and the
        temperature of the internal topk_selector. This is useful for
        adaptive temperature scheduling during training.

        Args:
            new_temperature (float): New temperature value (must be positive)

        Example:
            >>> selector = WeightedTopKSelector(k=10, temperature=1.0)
            >>> selector.update_temperature(0.5)  # Make selection "harder"
        """
        if new_temperature <= 0:
            raise ValueError(f"Temperature must be positive, got {new_temperature}")

        self.temperature = new_temperature
        self.topk_selector.temperature = new_temperature
