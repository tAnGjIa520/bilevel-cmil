...
class Training():
    ...
    def __init__(self, proxy_model, beta, device, lr_proxy_model, lr_weights, distall_lamda=0.1, draw_curve=False, topk=10, topk_method='sigmoid', topk_temperature=0.1, normalize_method='none', distill_target='logits', temp_scheduler=None, weight_optimizer_type='sgd', weight_adam_lr=0.001, weight_adam_betas=(0.9, 0.999), weight_adam_eps=1e-8, lr_scheduler=None, neumann_series_depth=3):
        """
        初始化训练模块
        ...
        参数:
            ...
            temp_scheduler: 温度调度器 (TemperatureScheduler 实例，可选)
            weight_optimizer_type: 权重优化器类型 ('sgd' 或 'adam')
            weight_adam_lr: Adam 学习率
            weight_adam_betas: Adam betas (动量系数)
            weight_adam_eps: Adam epsilon (数值稳定性)
            lr_scheduler: 学习率调度器 (LearningRateScheduler 实例，可选)
            neumann_series_depth: Neumann级数深度 Q (二阶梯度近似迭代次数，默认值为3)
        """
        self.proxy_model = proxy_model
        self.origin_model = None
        self.lr_p = lr_proxy_model  # 代理模型的学习率
        self.lr_w = lr_weights  # w的学习率 (用于 SGD)
        ...
        self.topk = topk
        self.neumann_series_depth = neumann_series_depth  # Neumann级数深度 Q
        ...
        # Weight optimizer 配置
        self.weight_optimizer_type = weight_optimizer_type.lower()
        self.weight_adam_lr = weight_adam_lr
        self.weight_adam_betas = weight_adam_betas
        self.weight_adam_eps = weight_adam_eps
    ...
    def _call_model(self, model, x, label=None, return_features=False, seen_classes=None):
        model_type = type(model).__name__

        if 'TransMIL' in model_type:
            output = model(x)
        else:
            output = model(x, label=label, return_features=return_features, seen_classes=seen_classes)

        # ----------- 🔥 在这里统一修 logits 维度 🔥 ----------- #
        logits = output["logits"]

        # print("logits shape before reshape =", logits.shape)
        # exit()
        
        # 如果 logits 维度大于 2（例如 [B,1,C]），自动 reshape
        # if logits.dim() > 2:
        #     # 保留 batch 和类数，展平多余维度
    ...
    def train_inner(self, data_S, target_S, task_id, sample_weights, inner_epchos,seen_classes=None,topk=10):
        ...
        # 如果有 temperature scheduler，更新温度
        if self.temp_scheduler is not None:
            new_temp = self.temp_scheduler.get_temperature()
            self.weighted_topk_selector.update_temperature(new_temp)
        ...
        loss_value = 0.0
        loss_values = []  # 累积损失值，避免频繁 .item() 调用
        ...
        for i in range(inner_epchos):
            ...
            # 保存损失值用于返回（不调用 .item()，保持在 GPU）
            loss_value = loss.detach()
            loss_values.append(loss_value)

        # 批量转换最后的损失值（单次 GPU-CPU 同步而非多次）
        final_loss_value = loss_value.item()

        # 打印内层循环的损失（使用累积的值，不触发额外同步）
        # 注意：这里延迟打印，使用前一次迭代的值，避免等待当前 GPU 计算完成
        if len(loss_values) > 0:
            loss_to_print = loss_values[-1].item()
            print(f"  └─ Inner[{inner_epchos}/{inner_epchos}]: {loss_to_print:.4f} ✓")

        # 返回最后一次迭代的损失（作为标量tensor，保持与原代码接口兼容）
        return torch.tensor(final_loss_value, device=self.device)
    ...
    def compute_outer_loss(self, input_train, target_train, input_selected, sample_weights, seen_classes=None):
        ...
        # 1. 计算代理模型在训练数据上的输出
        proxy_output = self._call_model(self.proxy_model, input_train, label=target_train,
                                       return_features=True, seen_classes=seen_classes)

        # print("logits shape =", proxy_output["logits"][:, seen_classes].shape)
        # exit()
        # target_train=target_train.argmax(dim=1)
        
        # 2. 计算分类损失（每个样本）
        if seen_classes is None:
            logits = proxy_output["logits"]
        else:
            logits= proxy_output["logits"][:, seen_classes]
            
        loss_outer = F.cross_entropy(logits, target_train, reduction='none')
        ...
        if self.origin_model is not None:
            ...
            # 原始模型输出（不需要梯度）
            with torch.no_grad():
                origin_output = self._call_model(self.origin_model, weighted_input, label=target_train,
                                                return_features=True, seen_classes=seen_classes)

            # 代理模型输出（需要梯度）
            new_output = self._call_model(self.proxy_model, weighted_input, label=target_train,
                                         return_features=True, seen_classes=seen_classes)
    ...
    def update_sample_weights(self, input_data, target_data, sample_weights, seen_classes=None):
        ...
        model_output = self._call_model(self.proxy_model, weighted_input, label=target_data,
                                       return_features=True, seen_classes=seen_classes)
        
        # import pudb; pudb.set_trace()
        
        loss_inner = torch.mean( F.cross_entropy(
            model_output["logits"][:,seen_classes], target_data, reduction='none'))
        ...
        v_Q = v_0
        for _ in range(self.neumann_series_depth):
        ...
        # 使用指定的优化器更新权重
        if self.weight_optimizer_type == 'adam':
            # 延迟初始化 Adam 优化器
            if self.weight_optimizer is None:
                sample_weights.requires_grad = True
                self.weight_optimizer = torch.optim.Adam(
                    [sample_weights],
                    lr=self.weight_adam_lr,
                    betas=self.weight_adam_betas,
                    eps=self.weight_adam_eps
                )

            # 使用 Adam 更新权重
            self.weight_optimizer.zero_grad()
            sample_weights.grad = gradient
            self.weight_optimizer.step()

            # 监控 Adam 权重更新（批量调用 .item()，单次 GPU-CPU 同步）
            current_lr = self.weight_optimizer.param_groups[0]['lr']

            # 收集所有需要打印的值，避免多次 GPU-CPU 同步
            values_to_print = {
                'gradient_norm': torch.norm(gradient),
                'weight_min': sample_weights.min(),
                'weight_max': sample_weights.max(),
                'weight_mean': sample_weights.mean()
            }
            # 批量转换（单次同步）
            values_cpu = {k: v.item() for k, v in values_to_print.items()}

            print(f"  ├─ [Adam] lr = {current_lr:.6f}")
            print(f"  ├─ [Adam] gradient_norm = {values_cpu['gradient_norm']:.6f}")
            print(f"  └─ [Adam] weights: min={values_cpu['weight_min']:.6f}, max={values_cpu['weight_max']:.6f}, mean={values_cpu['weight_mean']:.6f}")

            # 应用学习率调度（如果启用）
            if self.lr_scheduler is not None:
                new_lr = self.lr_scheduler.get_lr()
                self.weight_optimizer.param_groups[0]['lr'] = new_lr
                print(f"  ├─ Weight optimizer lr scheduled: {new_lr:.6f}")
                self.lr_scheduler.step()
        else:
            # 默认 SGD 更新
            with torch.no_grad():
                weight_change = self.lr_w * gradient
                sample_weights -= weight_change

                # 监控 SGD 权重更新（批量调用 .item()，单次 GPU-CPU 同步）
                # 收集所有需要打印的值，避免多次 GPU-CPU 同步
                values_to_print = {
                    'gradient_norm': torch.norm(gradient),
                ...
                print(f"  ├─ [SGD] lr_w = {self.lr_w:.6f}")
                print(f"  ├─ [SGD] gradient_norm = {values_cpu['gradient_norm']:.6f}")
                print(f"  ├─ [SGD] weight_change_norm = {values_cpu['weight_change_norm']:.6f}")
                print(f"  └─ [SGD] weights: min={values_cpu['weight_min']:.6f}, max={values_cpu['weight_max']:.6f}, mean={values_cpu['weight_mean']:.6f}")
