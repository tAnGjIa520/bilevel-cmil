...
class GPUScheduler:
    ...
    def __init__(self, gpu_ids: List[int], max_jobs_per_gpu: int = 1, wandb_project: str = "CMIL", wandb_config: Dict = None, exp_name_prefix: str = "hyperparam_search", notes: str = None):
        """
        初始化GPU调度器
        ...
        self.wandb_run = wandb.init(
            ...
            name=f"hyper_search_{exp_name_prefix}_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
        ...
        if table_data:
            ...
            if successful_jobs:
                ...
                if summary_data:
                    ...
                    # 将结果写入 wandb 表格
                    wandb_table = wandb.Table(columns=[param_name, "AACC", "BWT", "IM"])
                    for item in summary_data:
                    ...
                    wandb.log({f"{param_name}_results": wandb_table})

                    # 发送完成告警
                    alert_text = f"超参数搜索实验完成 ({param_name})！\n"
                    alert_text += f"成功任务数: {len(successful_jobs)}/{len(self.completed_jobs)}\n\n"

                    # 列出所有任务的详细结果
                    alert_text += "=" * 50 + "\n"
                    for item in summary_data:
                        alert_text += f"{param_name}={item[param_name]}: "
                        if 'AACC' in item:
